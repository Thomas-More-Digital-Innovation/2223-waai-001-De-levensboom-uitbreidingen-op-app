...
@section('content')
        ...
        @foreach ($tree_parts as $tree_part)
            <h2>
                <button type="button"
                    ...
                    onclick="toggleTestDiv({{ $tree_part->id }})">
                    <span>{{ $tree_part->name }}</span>
                    <svg id="svg-{{ $tree_part->id }}" data-accordion-icon class="w-3 h-3 shrink-0" aria-hidden="true"
            ...
            <div class="hidden test-div" id="test-{{ $tree_part->id }}">
    ...
    <div class="mt-10 flex gap-5 buttons">
        <a href="{{ route('clientLinks.edit', $client->id) }}"
            class="rounded bg-wb-blue px-5 py-2 text-white font-bold">Vorige pagina</a>
        <button onclick="window.print()" class="rounded bg-wb-blue px-5 py-2 text-white font-bold">Print</button>
    </div>
@endsection
