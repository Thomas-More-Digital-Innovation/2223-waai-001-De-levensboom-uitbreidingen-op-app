        ...
        if ($secretCodes->create()) {
            ...
            if ($stmt->rowCount() > 0) {
                ...
                if (preg_match('#\b(' . $pattern . ')\b#', $mailinhoud)) {
                    ...
                    $verifieerlink = substr($webAppLink  . "/src/pages/account/wachtwoordResetApp.php?id=" . $row['clientId'] . "&code=" . $randomNumber, 8);

                    $mailinhoud = str_replace("[verifieerlink]", $verifieerlink, $mailinhoud);
                    $mailinhoud = str_replace("[wachtwoordresetlink]", $verifieerlink, $mailinhoud);
